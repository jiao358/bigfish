<%@ page language="java" contentType="text/html; charset=utf-8"
    pageEncoding="utf-8"%>
<%@ page import="ext.core.domain.*" %>
<%@ page import="ext.datasource.entity.*" %>
<%@ page import="java.util.*" %>
<%
	BaseSys sys=(BaseSys)session.getAttribute("baseSys");
	List<FontModule> modules= sys.getModules();
%>
<!DOCTYPE html>
<html>
<head>
...
<body class="easyui-layout">
	...
	<div class="wu-header" data-options="region:'north',border:false,split:true">
        ...
        <div class="wu-header-right">
        	<p><strong class="easyui-tooltip" ><%=sys.getUserName() %></strong>，欢迎您！</p>
	...
	<div class="wu-sidebar" data-options="region:'west',split:true,border:true,title:'导航菜单'"> 
    	<div class="easyui-accordion" data-options="border:false,fit:true"> 
            ...
            <div title="学生管理" data-options="iconCls:'icon-application-cascade'" style="padding:5px;">  	
    			<ul class="easyui-tree wu-side-tree">
                	<li iconCls="icon-chart-organisation"><a href="javascript:void(0)" data-icon="icon-chart-organisation" data-link="temp/cus/customerIndex.html" iframe="0">学生</a></li>
                </ul>
            </div>
           <%for(FontModule root:modules){ 
           	  String title = root.getParent().getDesName();
           	  String  icon = root.getParent().getIcon();
           	   List<SModule> childrens = root.getChildrens();
           %>
           	  <div title="<%=title %>" data-options="iconCls:'<%=icon %>'" style="padding:5px;">  	
           	  <%if(childrens!=null && childrens.size()>0){ %>
           		<%for(SModule module:childrens){ %>
           		<% 
           			String subIcon = module.getIcon();
           			String subDes =module.getDesName();
           			String link ="temp/"+ module.getParent()+"/"+module.getModuleLink();
           			int iframe = module.getIframe()?1:0;
           		%>
          		 <ul class="easyui-tree wu-side-tree">
                	<li iconCls="<%=subIcon%>"><a href="javascript:void(0)" data-icon="<%=subIcon %>" data-link="<%=link %>" iframe="<%=iframe%>"><%=subDes %></a></li>
                </ul>
           	<%} %>
           	<%} %>
